...
def forecasts_dens(train, n_batch=1,n_lag=1, n_forecast=1,n_neurons=4):
    forecasts = list()
    X, y = train[:, 1:n_lag+1], train[:, 0]
    X=X.reshape(X.shape[0], X.shape[1])
    y=y.reshape(y.shape[0], 1)

    net = models.Sequential()                                                   #!!! model, kluczowe parametry modelu
   
    net.add(layers.Dense(n_neurons,activation='relu',input_shape=(n_lag,),
                         kernel_initializer=initializer1,bias_initializer=initializer3))
    
    net.add(layers.Dense(1,activation='linear', kernel_initializer=initializer1,bias_initializer=initializer3))
    net.compile(optimizer='adam',loss='mean_squared_error',metrics=['mae'])
    # fit network
    for i in range(n_epochs):
        net.fit(X, y, epochs=1, batch_size=n_batch, verbose=0, shuffle=False)
        net.reset_states()
    XX=X[len(X)-1].reshape(1, X.shape[1])
    forecast = net.predict(XX, batch_size=n_batch,use_multiprocessing=True,verbose=0)
    
    for i in range(n_forecast):
        XX=np.concatenate((pd.DataFrame(XX),pd.DataFrame(forecast)),axis=1)
        XX=XX[:,1:]
        forecast = net.predict(XX, batch_size=n_batch,use_multiprocessing=True,verbose=0)
...
def plot_forecasts(rate,forecasts,low,up,date):
    ...
    plt.title('Model DENS:  '+'  [lag='+str(n_lag)+']', loc='center')
...
for i in range(ntimes):
    scaler, train = prepare_data(rate, n_lag)
    # make forecasts
    forecasts=forecasts_dens(train, n_batch=n_batch, n_lag=n_lag, n_forecast=n_forecast,n_neurons=n_neurons)
